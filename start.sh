...
PDNS_ALLOW_UPDATE_FROM=${PDNS_ALLOW_UPDATE_FROM:-127.0.0.1}
PDNS_UPDATE=${PDNS_UPDATE:no}
PDNS_ALLOW_UNSIGNED_NOTIFY=${PDNS_ALLOW_UNSIGNED_NOTIFY:no}
...
sed -i "s/{{PDNS_ALLOW_UPDATE_FROM}}/${PDNS_ALLOW_UPDATE_FROM}/" /etc/powerdns/pdns.conf
sed -i "s/{{PDNS_UPDATE}}/${PDNS_UPDATE}/" /etc/powerdns/pdns.conf
sed -i "s/{{PDNS_ALLOW_UNSIGNED_NOTIFY}}/${PDNS_ALLOW_UNSIGNED_NOTIFY}/" /etc/powerdns/pdns.conf
